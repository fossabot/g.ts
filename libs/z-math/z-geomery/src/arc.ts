...
export namespace arc {
  ...
  export function nearAngle(angle, startAngle, endAngle, clockwise) {
    let plus = 0;
    if (endAngle - startAngle >= Math.PI * 2) {
      plus = Math.PI * 2;
    }
    startAngle = mod(startAngle, Math.PI * 2);
    endAngle   = mod(endAngle, Math.PI * 2) + plus;
  ...
  export function projectPoint(cx, cy, r, startAngle, endAngle, clockwise, x, y) {
    const rst = {};
    pointDistance(cx, cy, r, startAngle, endAngle, clockwise, x, y, rst);
    return rst;
  }

  export function pointDistance(cx, cy, r, startAngle, endAngle, clockwise, x, y, out) {
    const v    = [x, y];
    const v0   = [cx, cy];
    const v1   = [1, 0];
  ...
  export function box(cx, cy, r, startAngle, endAngle, clockwise) {
    const angleRight  = 0;
