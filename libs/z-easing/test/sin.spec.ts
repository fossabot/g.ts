...
tape('easeSinIn(t) coerces t to a number', function(test) {
  ...
  test.strictEqual(ease.easeSinIn({valueOf() { return 0.9; }}), ease.easeSinIn(0.9));
  test.end();
});

tape('easeSinOut(t) returns the expected results', function(test) {
  let sinOut = generic.out(ease.easeSinIn);
...
tape('easeSinOut(t) coerces t to a number', function(test) {
  ...
  test.strictEqual(ease.easeSinOut({valueOf() { return 0.9; }}), ease.easeSinOut(0.9));
  test.end();
});

tape('easeSinInOut(t) returns the expected results', function(test) {
  let sinInOut = generic.inOut(ease.easeSinIn);
...
tape('easeSinInOut(t) coerces t to a number', function(test) {
  ...
  test.strictEqual(ease.easeSinInOut({valueOf() { return 0.9; }}), ease.easeSinInOut(0.9));
  test.end();
});
