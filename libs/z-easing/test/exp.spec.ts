...
tape('easeExpIn(t) coerces t to a number', function(test) {
  ...
  test.strictEqual(ease.easeExpIn({valueOf() { return 0.9; }}), ease.easeExpIn(0.9));
  test.end();
});

tape('easeExpOut(t) returns the expected results', function(test) {
  let expOut = generic.out(ease.easeExpIn);
...
tape('easeExpOut(t) coerces t to a number', function(test) {
  ...
  test.strictEqual(ease.easeExpOut({valueOf() { return 0.9; }}), ease.easeExpOut(0.9));
  test.end();
});

tape('easeExpInOut(t) returns the expected results', function(test) {
  let expInOut = generic.inOut(ease.easeExpIn);
...
tape('easeExpInOut(t) coerces t to a number', function(test) {
  ...
  test.strictEqual(ease.easeExpInOut({valueOf() { return 0.9; }}), ease.easeExpInOut(0.9));
  test.end();
});
